...
def build_proxy_loader(config):
    dataset_proxy, nb_classes = build_dataset(is_train = False, config = config, is_proxy = True)
    print(
        f"local rank {config.LOCAL_RANK} / global rank {dist.get_rank()} successfully build proxy dataset")
    
    sampler_proxy = MyDistributedSampler(
        dataset_proxy, shuffle=False,
        drop_last=False, padding=False, pair=False,
    )
    ...
    return data_loader_proxy

    
def build_dataset(is_train, config, is_proxy = False):
    ...
    if config.DATA.DATASET == 'imagenet':
        ...
        if is_proxy: # create dataset with 1 image per classes
            ...
            for target in range(nb_classes):
                ...
                selected_indices.extend(class_indices[:config.NAS.PROXY.IMG_PER_CLASSES])
