...
def load_checkpoint(config, model, optimizer, lr_scheduler, loss_scaler, logger, candidate_pool = None):
    logger.info(
    ...
    if not config.EVAL_MODE:
        ...
        if candidate_pool is not None and 'candidate_pool' in checkpoint:
            candidate_pool_state_dict = checkpoint['candidate_pool']
            if candidate_pool_state_dict is not None:
                candidate_pool.load_state_dict(candidate_pool_state_dict)
...
def save_checkpoint(config, epoch, model, max_accuracy, optimizer, lr_scheduler, loss_scaler, logger, candidate_pool = None):
    save_state = {'model': model.state_dict(),
                  ...
                  'epoch': epoch,
                  'config': config,
                  'candidate_pool': candidate_pool.state_dict() if candidate_pool is not None else None}
