...
class SparseSwinTransformer(nn.Module):
    ...
    def random_config_fn(self, flops_constraint:tuple = None, max_patient_iters = 10000):
    ...
    def flops(self, cfg = None, precision = 2):
        ...
        # calculate the flops except the linear embedding part
        ...
        for i, layer in enumerate(self.layers):
            for blk in layer.blocks:
                ...
                flops += blk.dim * H * W
                 ...
                 # W-MSA/SW-MSA
                nW = H * W / blk.window_size / blk.window_size
                flops_attn = 0
                N = blk.window_size * blk.window_size
                # qkv = self.qkv(x)
                qkv_sparse_choice = config[block_cnts * 4 + 1]
                
                flops_attn += N * blk.dim * 3 * blk.dim * (qkv_sparse_choice[0] / qkv_sparse_choice[1])
                # attn = (q @ k.transpose(-2, -1))
                flops_attn += blk.attn.num_heads * N * (blk.attn.dim // blk.attn.num_heads) * N
                #  x = (attn @ v)
                flops_attn += blk.attn.num_heads * N * N * (blk.attn.dim // blk.attn.num_heads)
                # x = self.proj(x)
                proj_sparse_choice = config[block_cnts * 4]
                flops_attn += N * blk.dim * blk.dim * (proj_sparse_choice[0] / proj_sparse_choice[1])
                
                flops += flops_attn * nW
                
                # mlp
                fc1_sparse_choice = config[block_cnts * 4 + 2]
                flops += H * W * blk.dim * blk.dim * blk.mlp_ratio * (fc1_sparse_choice[0] / fc1_sparse_choice[1])
                fc2_sparse_choice = config[block_cnts * 4 + 3]
                flops += H * W * blk.dim * blk.dim * blk.mlp_ratio * (fc2_sparse_choice[0] / fc2_sparse_choice[1])
                # norm2
                flops += blk.dim * H * W
                block_cnts += 1
            if layer.downsample is not None:
               flops += layer.downsample.flops()
